...
def build_static_system_prompt(
    ...
    sections.append(
        ...
        "`reverse_proxy localhost:8080`, so your handlers match first — **except on "
        "the paths your own API serves**, which are reserved and always reach you: "
        "`/chat`, `/dl`, `/files`, `/health`, `/info`, `/pending`, `/runtime`, "
        "`/subagents`, `/telegram`, `/workspace`. A snippet claiming one of those is "
        "silently ignored: Caddy reloads without complaint and the route never takes "
        "effect, so pick a path outside that list (`/myapp`, `/dashboard`, …).\n"
